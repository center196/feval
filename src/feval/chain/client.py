...
def _address_string(value: Any) -> str | None:
    if isinstance(value, str):
        return value
    if isinstance(value, dict):
        for name in ("Id", "id", "address", "ss58"):
            if name in value:
                return _address_string(value[name])
    return None


def _model_commitment_extrinsics(
    info: Any, *, netuid: int
) -> list[tuple[int, str | None, ModelCommitment]]:
    """Decode model commitment intents from a block without trusting them as successful."""

    found: list[tuple[int, str | None, ModelCommitment]] = []
    for index, extrinsic in enumerate(_field(info, "extrinsics") or []):
        ...
        try:
            if int(arguments.get("netuid")) != netuid:
                continue
            commitment = ModelCommitment.from_chain_value(
                _decode_commitment_content(arguments.get("info"))
            )
        except (TypeError, ValueError):
            continue
        signer = _address_string(extrinsic.get("address"))
        if signer is None:
            signer = _address_string(extrinsic.get("account_id"))
        if signer is None and isinstance(extrinsic.get("signature"), dict):
            signer = _address_string(extrinsic["signature"].get("address"))
        found.append((index, signer, commitment))
    return found


def _successful_extrinsic_indices(events: Any) -> set[int]:
    successful: set[int] = set()
    failed: set[int] = set()
    for item in events or []:
        record = getattr(item, "value", item)
        ...
        if isinstance(phase, dict):
            index = phase.get("ApplyExtrinsic")
        elif phase == "ApplyExtrinsic":
            index = record.get("extrinsic_idx", record.get("extrinsic_index"))
        ...
        if not isinstance(index, bool):
            try:
                ...
                index = None
        if isinstance(event, dict) and isinstance(index, int):
            key = (event.get("module_id"), event.get("event_id"))
            if key == ("System", "ExtrinsicSuccess"):
                successful.add(index)
            elif key == ("System", "ExtrinsicFailed"):
                failed.add(index)
    return successful - failed


def scan_model_commitment_history(
    *,
    netuid: int,
    network: str,
    start_block: int,
    ...
    """Return successful Feval model commitments in a finalized inclusive block range.

    The block body identifies candidate extrinsics. Each candidate must have a
    matching ``System.ExtrinsicSuccess`` event at its exact extrinsic index, so
    a failed call can never establish priority and multiple writes in one block
    remain ordered. Historical storage is used only when a codec omits the
    signer. The RPC endpoint must retain archive state back to ``start_block``.
    """

    if start_block < 0 or end_block < start_block:
        raise ValueError("invalid commitment history block range")
    bt = _import_bittensor()
    sub = bt.Subtensor(network)
    history: list[dict[str, Any]] = []
    seen: set[tuple[str, int, str]] = set()
    for number in range(start_block, end_block + 1):
        info = sub.block_info(number)
        if info is None:
            raise RuntimeError(f"archive RPC returned no block info for finalized block {number}")
        candidates = _model_commitment_extrinsics(info, netuid=netuid)
        if not candidates:
            continue
        events = sub.query(("System", "Events"), [], block=number)
        successful = _successful_extrinsic_indices(events)
        snapshot: list[dict[str, Any]] | None = None
        for extrinsic_index, signer, commitment in candidates:
            if extrinsic_index not in successful:
                continue
            if signer is not None:
                ...
                key = (signer, number, commitment.model_digest)
                if key not in seen:
                    history.append(row)
                    seen.add(key)
                continue
            if snapshot is None:
                snapshot = _read_model_commitments_from_sub(sub, netuid=netuid, block=number)
            matches = [
                row
                for row in snapshot
                if row["commit_block"] == number
                and row["commitment"].model_digest == commitment.model_digest
            ]
            if not matches:
            ...
            for row in matches:
                key = (row["hotkey"], number, commitment.model_digest)
                if key not in seen:
                    history.append(row)
                    seen.add(key)
    return sorted(history, key=lambda item: (item["commit_block"], item["hotkey"]))
